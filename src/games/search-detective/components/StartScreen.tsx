...
export default function StartScreen({ content, onStart }: StartScreenProps) {
  const { intro, rounds, rules, clues } = content;
    ...
    <div className="sd-shell flex min-h-[calc(100vh-3rem)] items-center justify-center p-4">
      ...
      >
        ...
        <div className="mb-4 flex flex-col items-center gap-3">
          <div className="mlq-emoji-tile sd-hero-tile flex h-20 w-20 shrink-0 animate-float text-4xl">
            🕵️
          </div>
          <div className="sd-portal rounded-2xl p-3 text-sm leading-relaxed text-foreground">
            {intro.greeting}
          </div>
        </div>
        ...
        <div className="mb-4 text-center">
          ...
          <h1 className="sd-gradient-text text-2xl font-black">{intro.title}</h1>
        </div>
        ...
        <div className="mb-5 flex items-center justify-center gap-1.5 text-center text-xs text-muted-foreground">
          ...
          <span>
            검색 미션 {rounds.length}개 · 해결마다 +{rules.pointsPerCorrect}점 · 단서 도감{" "}
            {clues.length}종
          </span>
        </div>
        ...
        <button
          onClick={onStart}
          className="sd-btn-cta w-full rounded-xl py-3 text-base font-bold text-white"
        >
          {intro.startLabel}
        </button>
      </motion.div>
